...
static inline void __enable_fault_irq(void)  { __ASM volatile ("cpsie f"); }
static inline void __disable_fault_irq(void) { __ASM volatile ("cpsid f"); }
...
static inline void __CLREX(void)   { __ASM volatile ("clrex"); }
...
static inline void NVIC_SetHandler(IRQn_Type IRQnr, uint32_t ISRHandler)
{
	*((uint32_t*)(SCB->VTOR)+IRQnr+16) = ISRHandler;
}
